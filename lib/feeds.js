...
function dedupeItems(items) {
  ...
  return items.filter(item => {
    const url = item.url.replace(/#.*$/, '').replace(/\/$/, '').toLowerCase();
    const title = item.title.toLocaleLowerCase().replace(/[^\p{L}\p{N}]+/gu, ' ').trim();
...
async function getAiFeed({ force = false, fetchImpl = fetch, sources = defaultSources() } = {}) {
  ...
  const uniqueItems = dedupeItems(items)
    .sort((a, b) => (b.publishedAt || '').localeCompare(a.publishedAt || ''));
  cache = { fetchedAt: Date.now(), items: uniqueItems, errors };
  return cache;
}
