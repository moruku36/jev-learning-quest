// 一問一答の出題と学習進捗（間隔反復）
// 進捗は store.quizProgress = { [cardId]: { box, correct, wrong, last, due, lastConfidence } } に保存する。
...
const QUIZ_MISTAKE_REASONS = ['読み落とし', '知識不足', '設問要求とのずれ', '時間不足'];
...
function applyAnswers(store, answers, now = Date.now()) {
  ...
  for (const ans of answers) {
    ...
    const confidence = ['high', 'medium', 'low'].includes(ans.confidence) ? ans.confidence : null;
    const mistakeReason = QUIZ_MISTAKE_REASONS.includes(ans.mistakeReason) ? ans.mistakeReason : '';
    const prev = store.quizProgress[card.id] || { box: 0, correct: 0, wrong: 0 };
    const box = correct ? Math.min(prev.box + 1, MAX_BOX) : 0;
    const days = correct
      ? (confidence === 'low' ? Math.min(BOX_INTERVAL_DAYS[box], 3) : BOX_INTERVAL_DAYS[box])
      : 1;
    store.quizProgress[card.id] = {
      ...
      lastMistakeReason: correct ? '' : (mistakeReason || '知識不足'),
      last: new Date(now).toISOString(),
      due: new Date(now + days * DAY_MS).toISOString()
    };
    results.push({ card, correct, confidence, mistakeReason: correct ? '' : (mistakeReason || '知識不足') });
  }
  return results;
}
