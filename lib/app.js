...
const { QUIZ_EXAMS, QUIZ_SESSIONS, AI_READINGS } = require('./content');
const { quizStats, buildDeck, applyAnswers, MAX_DECK_SIZE } = require('./quiz');
...
function createApp({ mode, storage, publicDir, publicConfig = {}, configError }) {
  ...
  return async function handler(req, res) {
    ...
    try {
      ...
      // 3-b. 同梱教材の一覧（一問一答の進捗と AI レポートの読了状況つき）
      ...
      if (req.method === 'GET' && pathname === '/api/quiz/deck') {
        ...
        const mode = ['mix', 'new', 'review'].includes(q.get('mode')) ? q.get('mode') : 'mix';
        const count = Math.min(Math.max(parseInt(q.get('count'), 10) || 10, 1), MAX_DECK_SIZE);
        sendJson(res, 200, { cards: buildDeck(store, { exam, session, mode, count }) });
        return;
      }
      ...
      if (req.method === 'POST' && pathname === '/api/quiz/answers') {
        ...
        const newHistory = {
          ...
          notes: '',
          quizResult: { total: results.length, correct: correctCount, wrongIds: wrong.map(c => c.id) },
          understandingScore: evalRes.understandingScore,
          noulNeedsReview: evalRes.needsReview,
