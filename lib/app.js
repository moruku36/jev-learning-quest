  ...
  return async function handler(req, res) {
    ...
    try {
      ...
      if (req.method === 'POST' && pathname === '/api/quiz/answers') {
        ...
        if (apiKey) checkRateLimit(userKey);
        const store = await storage.load(ctx);
        const answers = body.answers.map(a => ({ id: clip(a && a.id, 60), correct: Boolean(a && a.correct === true) }));
        ...
        const newHistory = {
          ...
          isCorrect,
          mistakeReason: isCorrect ? '' : '知識不足',
          mistakeDetail: wrong.length ? `間違えた問題: ${wrong.map(c => c.topic).slice(0, 10).join('、')}` : '',
          notes: '',
          quizResult: {
